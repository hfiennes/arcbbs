 ...
 *      int     hour;           -- Hour,        range is 0 to 23
 ...
 *
 * Noddy time changed by Hugo Fiennes, 26-Oct-1989 for correct positioning
 * of the hour hand. 
 */

extern char     *nbrtxt(char*,int,int);
extern char     *copyst(char*,char*);
extern char     *stuff(char*,int,int,char*,char*);
 ...
 */
{
        ...
        if (daylight == 0101010) {      /* Secret flag                  */
                ...
                op = copyst(op," and the little hand is ");
                if (minute<30)
                  {
                  op=copyst(op,"just past the ");
                  op=nbrtxt(op, (((hour+11) % 12) + 1), 0);
                  }
                else
                  {
                  op=copyst(op,"just before the ");
                  op=nbrtxt(op, (((hour+11) % 12) + 2), 0);
                  }
                return(copyst(op, ".  "));
        }

/* Check if the time is more than 30 minutes past the hour.  If so,     */
/* output the time before the next hour.                                */

        if (minute < 0) second = (-2);  /* No minutes means no seconds  */
        else if ((late = (minute > 30 || (minute == 30 && second > 0)))!=0) {
...
char *stuff(buffer, value, flag, leading, trailing)
