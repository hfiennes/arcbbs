...
short  chat_map[24][80];
short  *chat_ptr[24];
int    chat_cursorx=0,chat_cursory=0;
wimp_w chat_handle;

extern void chat_clearmap(void),open_swindow(wimp_w,int,int), 
            server_msgproc(wimp_eventstr*,void*);

void chat_redraw(wimp_w handle)
  {
  ...
  while (more)
    {
    left  = (myredraw.g.x0-myredraw.box.x0)/16;    /* Find the text lines  */
    right = (myredraw.g.x1-myredraw.box.x0)/16;    /* we NEED to redraw    */
    top   =-(myredraw.g.y1-myredraw.box.y1)/32;    /* And then redraw them */
    bottom=-(myredraw.g.y0-myredraw.box.y1)/32;
    right++;
...
void chat_data(int ch)
  ...
  switch(ch)
    ...
    default:
      {
      if (ch>31) chat_ptr[chat_cursory][chat_cursorx]=ch;
      chat_data(9);
      break;
      }
    }
...
void window_init()
  ...
  wimpt_init("Chat");             /* Setup task */
  res_init("Chat");               /* Resource init */
  ...
  win_register_event_handler(chat_handle,server_msgproc,0);

  /* Route unknown/null events to that window */
  win_claim_idle_events(chat_handle);
  win_claim_unknown_events(chat_handle);       
...
void open_swindow(wimp_w wind,int x,int y)
  ...
  if (wind==chat_handle)
    ...
    temp.w=chat_handle; temp.i=-1;
    temp.x=temp.y=0;    temp.height=1<<24;
    temp.index=0;
...
void server_msgproc(wimp_eventstr *e,void *handle)
  ...
  switch(e->e)
      ...
      if(e->data.but.m.bbits==1 || e->data.but.m.bbits==4)   /* Left/right button */
        ...
        switch(window)
          ...
          case 3: /* Chat window, gain caret */
            ...
            temp.w=chat_handle; temp.i=-1;
            temp.x=temp.y=0;    temp.height=1<<25;
            temp.index=0;
    ...
    case wimp_EKEY:     /* Key pressed */
      {
      if (e->data.key.chcode==27 || e->data.key.chcode==1) exit(0);
      chat_data(e->data.key.chcode);
      break;
      }
